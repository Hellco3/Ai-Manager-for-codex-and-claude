      ...
      <div className="stage-card active">
        <div className="flex items-center gap-3 mb-4">
          ...
          <div className="flex rounded-lg bg-slate-900/50 p-0.5">
            <button
              ...
              className={`px-3 py-1.5 rounded-md text-xs font-medium transition-all ${
                mode === 'auto' ? 'bg-blue-500/20 text-blue-400 shadow-sm' : 'text-slate-500 hover:text-slate-300'
              }`}
            >
              {t.form.auto}
            </button>
            <button
              ...
              className={`px-3 py-1.5 rounded-md text-xs font-medium transition-all ${
                mode === 'semi-auto' ? 'bg-purple-500/20 text-purple-400 shadow-sm' : 'text-slate-500 hover:text-slate-300'
              }`}
            >
              {t.form.semiAuto}
            </button>
          ...
          <span className="text-xs text-slate-600">
            {mode === 'auto' ? t.form.autoDesc : t.form.semiAutoDesc}
          </span>
        </div>
        ...
        <div className="flex items-center justify-between mt-4">
          <span className="text-xs text-slate-600">
            {task.length > 0 ? `${task.length} ${t.form.chars}` : t.form.beSpecific}
          </span>
          <button
            type="submit"
