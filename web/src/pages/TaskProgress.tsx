...
import { cancelTask, getTask, reconstructSession } from '../api/client.js';
...
export default function TaskProgress() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // Auto-open drawer on mobile if there are messages
  ...
  const handleReconstruct = async () => {
    if (!sessionId) return;
    try { await reconstructSession(sessionId); } catch { /* ignore */ }
  };
    ...
    <div className="h-full flex flex-col px-4 sm:px-6 py-4">
      ...
      <div className="flex items-center justify-between mb-4 flex-wrap gap-3 shrink-0">
        ...
        <div className="flex items-center gap-4">
          ...
          {/* Re-plan button */}
          {(isComplete || isError) && (
            <button onClick={handleReconstruct} className="px-4 py-2 rounded-lg bg-purple-500/10 border border-purple-500/20 text-purple-400 text-sm hover:bg-purple-500/20">
              {t.chat.reconstruct}
            </button>
          )}
        </div>
      ...
      {showChat && (
        <>
          <button
            ...
            className="fixed bottom-6 right-6 z-30 xl:hidden w-14 h-14 rounded-full bg-blue-600 text-white shadow-lg shadow-blue-500/30 flex items-center justify-center hover:bg-blue-500 transition-all"
          >
            <svg className="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 10h.01M12 10h.01M16 10h.01M9 16H5a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v8a2 2 0 01-2 2h-5l-5 5v-5z" />
            </svg>
          ...
          <ChatPanel sessionId={sessionId!} variant="drawer" isOpen={chatOpen} onClose={() => setChatOpen(false)} />
        </>
      )}
